...
class Completer:
    """Custom completer for file path completion with full path support."""
    def __init__(self):
        self.matches = []
        self.current_text = ""
    ...
    def complete(self, text, state):
        if state == 0:
            ...
            if '/' in text:
                ...
                basename = text[last_slash + 1:] if last_slash >= 0 else text
            else:
                dirname = '.'
                basename = text
            ...
            try:
                ...
                if os.path.exists(dirname) and os.path.isdir(dirname):
                    ...
                    # Try to match partial directory
                    parent_dir = os.path.dirname(dirname) or '.'
                    partial_dir = os.path.basename(dirname)
                    if os.path.exists(parent_dir):
                        try:
                            matching_dirs = [d for d in os.listdir(parent_dir)
                                           if d.lower().startswith(partial_dir.lower()) and
                                           os.path.isdir(os.path.join(parent_dir, d))]
                            if matching_dirs:
                                # Use the first matching directory
                                matched_dir = matching_dirs[0]
                                if '/' in text:
                                    new_dir = os.path.join(os.path.dirname(dirname) or '.', matched_dir)
                                    # Get remaining files in this directory
                                    if os.path.exists(new_dir):
                                        items = os.listdir(new_dir)
                                        self.matches = [item + '/' if os.path.isdir(os.path.join(new_dir, item)) else item
                                                      for item in items]
                                        self.matches.sort()
                        except:
                            self.matches = []
        ...
        if state < len(self.matches):
            # Return the match with proper path
            match = self.matches[state]
            if '/' in self.current_text:
                last_slash = self.current_text.rfind('/')
                dirname = self.current_text[:last_slash] if last_slash > 0 else '.'
                if dirname.startswith('~'):
                    dirname = os.path.expanduser(dirname)
                # If the match already has a slash, don't add another
...
def setup_tab_completion():
    ...
    if readline:
        ...
        # Set delimiters to handle paths properly
...
def generate_domain_variants(company_name: str, country_hint: Optional[str] = None) -> List[str]:
    ...
    if ' ' in company_name:
        ...
        for ext in extensions[:5]:  # Try fewer extensions for hyphenated versions
...
def find_valid_domain(company_name: str, timeout: int = 3) -> Tuple[Optional[str], List[str]]:
    ...
    # Try each variant silently (no individual progress shown)
...
def generate_candidates(first, last, domain, include_numeric_suffixes=True):
    ...
    # Some companies append a number when a name is already taken
    # (e.g. a second "John Smith" becomes john.smith2@company.com).
    # Kept small and bounded on purpose - this is a real corporate
    # convention, not an open-ended brute force sweep.
...
def verify_email_smtp(mx_server, email, mail_from="verify@local-tool.com",
    ...
    try:
        ...
        try:
            ...
            pass  # STARTTLS not supported, continue without it
...
def process_single_name(full_name: str, domain: str, mx_server: str, verbose: bool = True) -> Optional[str]:
    ...
    for email in candidates:
        ...
        if result["status"] == "valid":
            return email
        time.sleep(0.5)  # be polite to the mail server

    return None


def get_file_path_from_user(prompt: str) -> Optional[str]:
    """
    Get file path from user with simple error handling.
    """
    try:
        file_path = input(f"\n{prompt}").strip()
    ...
    # If user entered nothing, return None
    if not file_path:
        return None

    # Clean the path - remove any extra quotes that might have been added
    ...
    if retry == 'y':
        return get_file_path_from_user(prompt)
    return None
...
def batch_mode():
    ...
    print("BATCH MODE")
    print("=" * 50)
    print("\n[!] Press TAB to auto-complete file paths")

    # Setup tab completion for file input
    setup_tab_completion()

    # Get file path with simple handling
    ...
    # Get company name once for all names
    try:
    ...
    if check_catch_all(mx_server, domain):
        ...
        # Just show best guess for each name
        results = {}
        for name in names:
...
""")
    ...
    # Ask for mode with keyboard interrupt handling
    try:
        print("Select mode:")
        ...
        if choice == '1':
            ...
            # Recursively call main to try again
            main()
    except KeyboardInterrupt:
        print("\n\n[!] Operation cancelled by user.")
